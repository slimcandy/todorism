...
export const MembersPage = () => {
    ...
    const [deletingMember, setDeletingMember] = useState<{ name: string, member_uid: string }>({
        name: "",
        member_uid: ""
    });
    ...
    const [list, setList] = useState<Array<{ name: string, member_uid: string }>>(membersList);
...
//     const trip_uid = eventUid ?? "";
    ...
    const onRemoveMemberFromList = () => {
        const newList = list.filter(m => m.member_uid !== deletingMember.member_uid)
        setList(newList)
    }
        ...
        >
            <div>
                ...
                <MembersList list={list} onEdit={setEditingMember}
                             onEditClick={setIsEditing} onDelete={setDeletingMember}/>
                {/*        {list.map((member) => (
          <div className="mb-2">
            <MembersListItem
            ...
            <div className="flex items-center justify-between">
                <div className="mr-6 w-full">
                    <Input value={isEditing ? editingMember.name : ""} placeholder="ещё участник"
